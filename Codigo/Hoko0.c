...
extern int LM, LT, LKL, LKS, JREZ[MAXREZ];
extern double
...
double
    ...
    f1,
    B3, B3F1, PS1, B2, SB2, CB2, AL, A1, D, SL1, CL1, SL, CL, S2L, C2L, S3L,
    C3L, SF, CF, S2F, C2F, SD, CD, S2D, C2D, S4D, C4D, SLM2D, CLM2D, SLP2D,
    CLP2D, SLMD, CLMD, S2LMD, C2LMD, SPLM2D, CPLM2D, S1M2D, C1M2D, S1P2D, C1P2D,
    SLM1, CLM1, SLP1, CLP1, SLM4D, CLM4D, S2LM4D, C2LM4D, S2LPD, C2LPD, S1PD,
    S3LM2D, S2LM1, SLM1M2, CLM1M2, SLM1P2, CLM1P2, SLM2F, CLM2F, SLP2F, S2FMD,
    SLM2FD, SLPF, SLMF, SFM2D, CFM2D, SFP2D, SF2DML, SF2DPL, S2LPF, SFM2L,
    SFML2D, SFPL2D, SFP12D, S2LDF, S2DL1F, SFM2D1, SFM12D, dl, S1, O, B4, SB4,
    CB4, SS1, CS1, CSSB, B, S2, C2, S3, C3, S4, C4, DLS, DR4, R4, R4D, SB, CB,
    BET

    ,
    Xl, Yl, Zl, Xc, Yc, Zc, kappa, omega, nu;

/*########################## Function KOOPLS   ###########################*/
/* ======================================================================
        ??????????: ??????????? ????????? ???? ? ?????? ?? ???????? ??????
                                                        ???????.
        ????? ?????????: KOOPLS(TM,XL,YL,ZL,XC,YC,ZC)
        ??????? ??????????:
       ?M - ?????, ????????????? ?? 0h 31 ??????? 1957 ?. (?????),
        ???????? ??????????:
        ?????????? ???? ? ??????: XL,YL,ZL,XC,YC,ZC ? ???? (??.)
...
/* ======================================================================
         ...
         Input data: ?M is the time in days from 0h UTC Dec 31, 1957.
         Output data:
         Coordinates of the Moon (L) and Sun (C) in km: XL,YL,ZL,XC,YC,ZC.
     ------------------------------------------------------------------- */
/**
@brief Calculation of the Moon and Sun coordinates at requestion time.
@param TM Time in days from 0h UTC Dec 31, 1957.
@param XL X coordinate of the Moon in km.
@param YL Y coordinate of the Moon in km.
@param ZL Z coordinate of the Moon in km.
@param XC X coordinate of the Sun in km.
@param YC Y coordinate of the Sun in km.
@param ZC Z coordinate of the Sun in km.
@returns void

*/
void JVC_DLL KOOPLS(double TM, double far *XL, double far *YL, double far *ZL,
                    double far *XC, double far *YC, double far *ZC) {

  static double AL0, A10, F10, D0, DT00, B3O, ALSO, P1, PS, AM1, AM2, AM3, AM4,
      ANU, AE1, AE2, AE3, AE4, sinB2, cosB2, DI;
  double P[8], d_mu, d_nu, d_epsilon;

  if (fabs(DT00 - DT0) > 1.0e-9) {
    double B0, T1, T2, T3, EP, ES, AE, EZ, ED, EV;
    DT00 = DT0;
    B0 = DT0 + 21183.5 + TDT_TAI + DAT;
    T1 = B0 / 36525.0;
    T2 = T1 * T1;
    T3 = T1 * T2;
    EP = (23.452294 - 0.0130125 * T1 - 0.164 * T2 * 1.0E-5 +
          0.503 * T3 * 1.0E-6) /
         RADIAN;
  ...
  dl = (22639.58 * SL - 4586.438 * SLM2D + 2369.899 * S2D + 769.021 * S2L -
        668.944 * SL1 - 411.614 * S2F - 211.658 * S2LMD - 206.219 * SPLM2D +
        191.954 * SLP2D - 165.351 * S1M2D + 147.878 * SLM1 - 124.785 * SD -
        109.804 * SLP1 - 55.174 * S2FMD - 45.1 * SLP2F - /*PROVERIT ZNAK - */
        39.532 * SLM2F - 38.428 * SLM4D + 36.134 * S3L - 30.773 * S2LM4D +
        28.511 * SLM1M2 - 24.451 * S1P2D + 18.609 * SLMD + 18.023 * S1PD +
        14.577 * SLM1P2 + 14.387 * S2LPD + 13.902 * S4D - 13.193 * S3LM2D +
        9.703 * S2LM1 + 9.366 * SLM2FD) *
       r5;
  S1 = (18461.48 * SF + 1010.18 * SLPF + 999.695 * SLMF - 623.658 * SFM2D +
        199.485 * SF2DML - 166.577 * SF2DPL + 117.262 * SFP2D + 61.913 * S2LPF -
        33.359 * SFML2D - 31.763 * SFM2L - 29.689 * SFP12D - 15.565 * S2LDF +
        15.122 * SFPL2D + 8.902 * S2DL1F + 12.14 * SFM2D1 + 8.001 * SFM12D) *
       r5;
  O = (3422.7 + 186.5398 * CL + 34.3117 * CLM2D + 28.2333 * C2D +
       10.1657 * C2L + 3.0861 * CLP2D + 1.9202 * C1M2D + 1.4455 * CPLM2D +
       1.1542 * CLM1 - 0.9752 * CD - 0.9502 * CLP1 - 0.7136 * CLM2F +
       0.6215 * C3L + 0.6008 * CLM4D - 0.3997 * CL1 + 0.3722 * C2LM4D -
       0.3039 * C2LMD - 0.3 * C1P2D + 0.2833 * C2LPD + 0.2607 * C4D -
       0.2257 * CLM1M2 + 0.2302 * CLM1P2) *
      r5;
  B4 = B3 + dl - PS1;
  SB4 = sin(B4);
  CB4 = cos(B4);
...
/* ======================================================================
         ...
         ??????????? ???????:
         ??????? ??????????:
                X=-1.03811685765, Y=-.101398483053, Z=.038430850163, L?=5.
         ???????? ??????????:
                HLC[1,1]=-.97921271, HLC[2,1]=-.124726493,
   HLC[3,1]=1.6685198677, HLC[4,1]=-.122835649, HLC[5,1]=1.348477187,
   HLC[6,1]=.155723805, HLC[7,1]=-1.6842145413, HLC[8,1]=8.9581971e-1,
   HLC[9,1]=2.0965921e-1, HLC[10,1]=-1.31375875 ? ?.?. HLC[1,2]=0.,
   HLC[2,2]=-1.21827e-2, HLC[3,2]=3.29086e-1, HLC[4,2]=0., HLC[5,2]=1.31713e-1,
   HLC[6,2]=3.071379e-2, HLC[7,2]=-5.0644447e-1, HLC[8,2]=0.,
   HLC[9,2]=2.04785e-2, HLC[10,2]=-2.591159e-1 ? ?.?.
         ------------------------------------------------------------------- */
/* ======================================================================
     ...
     (see formula (1.3.38), Topic 5.2, Report 1).
         ...
         Input data:
        Coordinates: X,Y,Z.
                Maximum order of spherical functions to be calculated - L?.
         Output data:
...
;
/**
@brief Calculation of spherical functions value.
@param TM Time in days from 0h UTC Dec 31, 1957.
@param X X.
@param Y Y.
@param Z Z.
...
void JVC_DLL UV(double X, double Y, double Z, int LK, double far HLC[]) {
  int L, N, I, T, T1, A, B, C, K, T2;
  long SA, SB;
  double R, UD, VD, F, R1, RK;
  double far *U;
...
/* ======================================================================
   ...
   ??????????????? ?????????????? ?????????? GME, ????????  ???????? ????? CBZ,
   ?????????????? ?????? ????? RZ, ?????? ????????????? ?????????? ?????????????
   HK1[1:300,1:2]. ???????? ??????????: JREZ[1:7], QC[1:7,-20:20],
   QD[1:7,-20:20], PC[1:7,-20:20], PD[1:7,-20:20], RC[1:7,-20:20],
   RD[1:7,-20:20], TC[1:7,-20:20], TD[1:7,-20:20]. ???????????? ????????? ?
   ?????????? FINC,HANSEN. ??????????? ???????: ??????? ??????????:
...
;
/**
@brief Calculation of slow-changing resonance functions.
@returns void

*/
void JVC_DLL REZ(void)
/* ????? ??????? ????????? */ {
  double N, Z, A1, BD, AD, K1, H2, F1, F2, G1, G2;

  int M, S, P, K, J, J1 = 0, Q, L;
    ...
    for (M = 1; M <= LT; M++) {
      ...
      if (fabs(Z) < 5.0E-2) {
        ...
        for (Q = J - LT; Q <= J + LT; Q++) {
          ...
          ;
          QC[J1] = QD[J1] = RC[J1] = RD[J1] = PC[J1] = PD[J1] = TC[J1] =
              TD[J1] = 0.0;
          S = abs(J - Q);
          S = MAX(M, S);
          for (L = S; L <= LM; L++) {
...
/* ======================================================================
   ...
   ??????? ??????????:
    ?????????????? ?, ?????????? I,????? ???????????????? ?????? - L?.
   ???????? ??????????:
    ??????? L?[2:L?, 0:L?,0:L?,], Li[2:L?, 0:L?,0:L?,], Lv[2:L?, 0:L?,0:L?,],
    L?[2:L?, 0:L?,0:L?,],L?[2:L?, 0:L?,0:L?,].
   ???????????? ????????? ? ?????????? FINC,HANSEN.
   ??????????? ???????:
   ??????? ??????????:
    E=.0099371046, I=.523587802, L?=5.
   ???????? ??????????:
...
Otput data:
 Dynamic arrays: LE[0:(LLUN-1)*(LLUN+1)*(LLUN+1)-1],
LI[0:(LLUN-1)*(LLUN+1)*(LLUN+1)-1],
...
;
/**
@brief Calculation of slow-changing functions for taken into account perturbations due to attraction of the Moon and Sun.
@returns void

*/
void JVC_DLL LS(void) {
  double H1, H2, S, F1, F2, T1, T11, T2, T3, T4, T5, T6, T7;
  int L, T, M, P, J = 0;
  S = sqrt(1 - E * E);
  T1 = -S / E;
  T3 = 1.0 / (4 * S * SI2);
...
/* ======================================================================
   ...
   ------------------------------------------------------------------- */
	 /**
	 @brief Approximation of spherical functions.
	 @param DT step of approximation.
	 @param T X is initial time.
	 @param TK Y is final time.
	 @param THAX
	 @param TKOH
	 @returns void

	 */
void JVC_DLL APPROK(double DT, double T, double TK, double far *THAX,
                    double far *TKOH) {
  double far *ML1, far *ML2, far *ML3, far *ML4, far *MC1, far *MC2, far *MC3,
      far *MC4;
  double XL, YL, ZL, XC, YC, ZC, X;
  int K;
  if ((T < *THAX) || (T > *TKOH)) {
    if (TK >= 0) {
      *THAX = T;
    ...
    if ((ML1 = (double far *)calloc(LKL, sizeof(double))) == NULL ||
    ...
    for (K = 0; K < LKL; K++) {
      HL1[K] = (-25.0 / 12.0 * HL0[K] + 4 * ML1[K] - 3 * ML2[K] +
                4.0 / 3.0 * ML3[K] - 0.25 * ML4[K]) /
               DT;
      HL2[K] = 0.5 *
               (35.0 / 12.0 * HL0[K] - 26.0 / 3.0 * ML1[K] + 9.5 * ML2[K] -
                14.0 / 3.0 * ML3[K] + 11.0 / 12.0 * ML4[K]) /
               (DT * DT);
      HL3[K] = (-2.5 * HL0[K] + 9 * ML1[K] - 12 * ML2[K] + 7 * ML3[K] -
                1.5 * ML4[K]) /
               (6 * DT * DT * DT);
      HL4[K] = (HL0[K] - 4 * ML1[K] + 6 * ML2[K] - 4 * ML3[K] + ML4[K]) /
               (24 * DT * DT * DT * DT);
      if (K < LKS) {
        HC1[K] = (-25.0 / 12.0 * HC0[K] + 4 * MC1[K] - 3 * MC2[K] +
                  4.0 / 3.0 * MC3[K] - 0.25 * MC4[K]) /
                 DT;
        HC2[K] = 0.5 *
                 (35.0 / 12.0 * HC0[K] - 26.0 / 3.0 * MC1[K] + 9.5 * MC2[K] -
                  14.0 / 3.0 * MC3[K] + 11.0 / 12.0 * MC4[K]) /
                 (DT * DT);
        HC3[K] = (-2.5 * HC0[K] + 9 * MC1[K] - 12 * MC2[K] + 7 * MC3[K] -
                  1.5 * MC4[K]) /
                 (6 * DT * DT * DT);
        HC4[K] = (HC0[K] - 4 * MC1[K] + 6 * MC2[K] - 4 * MC3[K] + MC4[K]) /
                 (24 * DT * DT * DT * DT);
      };
    }
    free(ML1);
...
;
/**
@brief Calculation of shadow entry and exit parameters.
@param al
@param del
@param AP
@param DBY
...
*/
void JVC_DLL SHADOW(double al, double del, double AP, double DBY,
