...
//#define EPOCHA_IS_QUASINERTIAL   // It is used for coordinate system
//determination
...
#define LGEM                                                                   \
  (LMAX + 1) * (LMAX + 2) -                                                    \
      6 // Dimension of array for store gravity field coefficients
#endif
#define sqr(x) ((x) * (x))
...
#define LLUN                                                                   \
  5 // Maximum order of terms considered in expansions of the Moon disturbing
    // function
#define LSUN                                                                   \
  3 // Maximum order of  terms  in expansions of the Sun disturbing function
#define PI M_PI
...
#define CBZ                                                                    \
  6.300387486753 // Rotational velocity of the Earth in the quasi-inertial
                 // coordinate system
#else
#define CBZ                                                                    \
  6.30038738 // Rotational velocity of the Earth in the Mean Equinox coordinate
             // system
#endif
...
#define CTPOBFORCE                                                             \
  0.005 // Parameter, which determines how often will be recalculated slow
        // changing functions of averaged equations
#define CTPOBKP                                                                \
  0.005 // Parameter, which determines how often will be recalculated slow
        // changing functions of short periodics
#define EPSKP                                                                  \
  1.0E-8 // Parameter, which determines the value of short periodical amplitudes
         // for taken into account
...
struct bxprog { /*Name of structure*/
  ...
  double DT,    /*Initial time counted off from 0hUTC Dec 31 1957 in days*/
      ...
      F135,     /*The weighting-averaged value of solar activity index F10,7 for
                   preceding 81 days F10,7*/
  ...
  int PRMODEL,  /*Parameter for determining motion model*/
                /*
                 if PRMODEL=0 then it is chosen full model: (l,m)<=(LMAX,LMAX),
                 atmospheric drag, attraction of the Moon and Sun, solar radiation
                 pressure           if PRMODEL=1 then it is taken into account perturbations due
                 second zonal harmonic           if PRMODEL=n then it is chosen full model:
                 (l,m)<=(n,n), atmospheric drag, attraction of the Moon and Sun, solar
                 radiation pressure
                */
      POCK,     /*Parameter for determining form of input and ouput data*/
                /*
                 if POCK=0 then input and output elements are osculating ones
                 if POCK=-1 then input elements are mean elements, output elements are
                 osculating ones           if POCK=-2 then input and output elements are mean ones
                */

      PZADACHI; /*Parameter for determining the solvable prediction task*/
                /*
                 if PZADACHI=0 then "prediction to requestion time";
                 if PZADACHI=1 then "prediction to ascending node of preset revolution
                 number";               if PZADACHI=2 then "prediction to ascending node of current
                 revolution";               if PZADACHI=3 then satellite track calculation;               if PZADACHI=4
                 then transformation of RSSS input elements to Keplerian format;
                */
};
...
struct bixprog {
  ...
  double PER,        /*Nodal period, in min*/
      DT,            /*Change in nodal period due to the atmospheric drag
                        (min/revolution)*/
...
double JVC_DLL ATM(double H1, double F135, double F, int F0, double KP,
                   double T0);
...
int JVC_DLL PROGNOZ(int NTOCH, struct bxprog far *BX, struct bixprog far *BIX,
                    double far TKNK[]);
int JVC_DLL PROGNOZCHICL(int NTOCH, struct bxprog far *BX,
                         struct bixprog far *BIX, double TKNK[])
