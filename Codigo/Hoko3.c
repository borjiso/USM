...
extern int LM, LT, LKL, LKS, JREZ[MAXREZ], L, M, PSUNDAB, PATMOSFERA, PLUNASUN
...
extern double
    ...
    A,
    E, I, SI, CI, SI2, CI2, CE, SE, EA, IA, CIA, SIA, DT0, RO, EK[6], KA[6][5],
    KC[5][2], S0, HP, ALTITUDE, LONGITUDE, DTREENTRY;

extern double F135, FT, KP, KB0, AX, EX, IX, AG, EG, IG
...
int F0 = 150, PZADACHI, NPRABCHACTEI;

double RAD, DTBAKB = 0, PADEHIE = 0, LIFETIME, DTSTART, CE, SE, EA, IA, CIA,
            SIA, PER, DTK, AAL, AAC, TK, THAX, TKOH, K1_SUN, K2_SUN, K3_SUN,
            K4_SUN, K5_SUN, T_SUN = 1000000, SHAG
#if HOKO
//		,  SHAG=10.0 /*��� ������ 10*/
#else
//		,  SHAG=1.0 /*��� ������� 0.5*/
#endif
    ,
            CH = 0.5, ER[6], PQ0[6], PQ1[6], PQ2[6], PQ3[6], PQ4[6], Q1[6],
            E1[6], ER0[6], EKP[6], CW[MAX(2 * LMAX + 1, 7)],
            SW[MAX(2 * LMAX + 1, 7)];
...
/* ======================================================================
  ...
  ------------------------------------------------------------------- */
/**
 * @brief Prediction of satellite motion
 * @param  NTOCH number of prediction points.
 * @param  BX    initial vector of orbital elements
 * @param  BIX   BIX[0:NTOCH-1] is array of predicted orbital elements.
 * @param  TKNK  TKNK[0:NTOCH-1] is prediction mooments.
 * @return       int
 */
#ifdef HOKODLL
extern "C" int _export _stdcall PROGNOZ(int NTOCH, struct bxprog *BX,
...

{
  ...
  */
  ...
  int XAXA = 0, POCK = 0, PRMODEL = 0, J, JD, DIR, DIRSHAGA = 0,
      NT = 0 /*����� ����� ��������*/
      ,
      LMOLD = 0, LTOLD = 0, OLDMODEL, JQC = MAXREZ * (2 * LMAX + 1),
      JQC_double = JQC * sizeof(double),
      JLE = (LLUN - 1) * (LLUN + 1) * (LLUN + 1),
      JLE_double = JLE * sizeof(double);
  double FOLDMODEL;
  double A0, BETTA, P, I0, EKC, L0, H0, U0, W, MA, DBY, U, H,
      DA = 0, DKB = 0, DDT = 0, DT = 0, Dt = 0, T = 0, CH2, DTFINISH, DM, DH,
      C1, C2, C3, TKP = 10E10, E0[6];

  time_t time1, time2;

  int PKYTTA = 0, POCKYL = 0, POCKYLOUT = 0, PPC = 1;
 ...
 * ��������**********************************/
...
#if BOKO
  if ((QC = (double far *)calloc(JQC, sizeof(double))) == NULL ||
...

  {
    ...
    if (fabs(DT0 - PQ1[2]) > 1.0E-8)
      PPC = 0;
    if (fabs(KB0 - PQ1[3]) > 1.0E-8)
      PPC = 0;
    if (fabs(KB0 - PQ0[5]) > 1.0E-8)
      PPC = 0
          /*********�������� ���������� ������� ��������***********/
          ;
    if (PRMODEL != OLDMODEL)
  ...
  if (!DTBAKB) {
    ...
    P = (1 - Q1[1] * Q1[1]);
    PER *=
        1 + 1.5 * C20 * sqr(RZ / (P * ER[0])) * sqrt(P) *
                ((2 - 2.5 * sqr(sin(Q1[2]))) * P / sqr(1 + Q1[1] * cos(Q1[4])) +
                 (1 - 1.5 * sqr(sin(Q1[2]))));
  }
  SHAG = 10;
  if (SHAG * PER > 1.0)
  ...
  do {
    ...
    do /*���� ��� ���������*/
    {
      do /*���� ���  ������ �� U=0*/
      {
        ...
        if (DIRSHAGA == 0)
          ...
          DIRSHAGA = sign(DTFINISH - DTSTART);
        ;
        if ((PKYTTA) || ((DT0 + T + TK - DTSTART) * DIRSHAGA < 0) ||
            ((DTFINISH - DT0 - T - TK) * DIRSHAGA < 0) ||
            (PRMODEL != OLDMODEL)) {
          ...
          do {
            CH = H
                ...
                ;
            if (PLUNASUN &&
                ((T > TKOH) || (T < THAX) || (T + H > TKOH) || (T + H < THAX)))
            ...
            if ((CH - TK) * DIR < -1.0e-9) {
              ...
              for (J = 0; J <= 5; J++)
                ER[J] +=
                    CH * (PQ1[J] + 2.0 * PQ2[J] + 2.0 * PQ3[J] + PQ4[J]) / 6;
              ER[5] = fmod(ER[5], PI2);
              TK -= CH;
              T += CH;
            }
          ...
          if (LM > 1)
            fwrite(KC, sizeof(KC), 1, MAEIFORCE);
          ;
          if (LM > 1) {
        ...
        for (J = 0; J <= 5; J++)
          Q1[J] =
              ER[J] + CH * (C1 * PQ1[J] + C2 * (PQ2[J] + PQ3[J]) + C3 * PQ4[J]);
        Q1[5] = fmod(Q1[5], PI2);
        time2 = time(NULL)
#if PECHAT
            ;
    ...
    ;
    for (J = 0; J <= 5; J++)
      ER0[J] = ER[J] + CH * (PQ1[J] + 2.0 * PQ2[J] + 2.0 * PQ3[J] + PQ4[J]) / 6;
    ...
    DTK = DT0 + T;
    if ((((PZADACHI == 0) && (fabs(DTK - DT0) < 0.5E-9)) ||
         ((PZADACHI == 1) && (BIX->bix.NH == BX->NH) && U0 == 0) ||
         ((PZADACHI == 2) && (U0 == 0)) || (PZADACHI == 4)) &&
        POCKYL == POCKYLOUT) {
      E1[0] = A0;
      E1[1] = EKC;
      E1[2] = I0;
    ...
    EA = KEPLER(EK[5], EK[1]);
    BIX->Y[5] = EK[3] + 2 * atan2(sqrt((1 + EK[1]) / (1 - EK[1])) * sin(EA / 2),
                                  cos(EA / 2));
    ...
    */
    {
      ...
      P = (1 - EK[1] * EK[1]);
      PER *= 1 + 1.5 * C20 * sqr(RZ / (P * EK[0])) * sqrt(P) *
                     ((2 - 2.5 * sqr(sin(EK[2]))) * P / sqr(1 + BIX->Y[3]) +
                      (1 - 1.5 * sqr(sin(EK[2]))));
    }
    ...
    if (PZADACHI == 2 || PZADACHI > 3)
      BIX->bix.NH = BX->NH
          ...
          ;
    BIX->HA =
        BIX->bix.A * (1 + BIX->E) - RZ * (1 - ECJ * sqr(sin(I0) * sin(BIX->W)));
    HP =
        BIX->bix.A * (1 - BIX->E) - RZ * (1 - ECJ * sqr(sin(I0) * sin(BIX->W)));
  ...
  MREENTRY:;
    ...
    if (PZADACHI == 3) {
      ...
      else if ((xp[2] * sign(DTK - DT0) > 0) &&
               (Z0 * sign(DTK - DT0) < 0)) /* && (pz!=1)  && (pz!=2))*/
        Nk += DIR;
      Z0 = xp[2];
      if (LONGITUDE > 180)
        LONGITUDE -= 360;
...
/* ======================================================================
   ...
   Output data: E2[0:5] is vector of right-hand side of averaged equations
  value.
  ------------------------------------------------------------------- */

int JVC_DLL FORCE(double T, double E1[], double E2[]) {
  double Z, X, H1, H2, A1, A2, GR, L1, L2, L3, L4, L5, L6, N, MK, AK, K1, SK,
      CK, FR, SN, K2, K3, K4, K5, AP, DBY, CA, DOM, DOW, DM, C1, S1, JK, KJ, UG,
      EATM, HODH, B, C2, C3, B1, B2, FF, N2;
  int K, J0, J1, J, L, M, P, WQ, J10, J11, Q;
  ...
  if (PATMOSFERA) {
    ...
    if (J0 == -1) {
      ...
      if (HP < 80) {
        ...
        LONGITUDE =
            DBY - S0 - CBZ * T + atan2(sin(UG) * CI, cos(UG)) /* �GROTA*/
            ;
        if (LONGITUDE < -180)
          LONGITUDE += 360;
        if (LONGITUDE > 180)
      ...
      for (K = 0; (fabs(F135 - MF0[K]) > fabs(F135 - MF0[K + 1]) && (K <= 6));
           K++)
        ;
      F0 = MF0[K];
      AT1 = (AT + K * 75 + J0 * 25);
      ...
      FF = AT1[9] + AT1[10] * HP + AT1[11] * HP * HP + AT1[12] * HP * HP * HP
          //				  ;  N2=0.5*AT1[13]     //����
          ;
      N2 = 0.5 * (AT1[13] + 0.006 * (HP + 0.5 * A * E)) //����� �����������
                                                        //����������� n ��
                                                        //������ n1 �����
                                                        //���������� 0.006
          ;
...
#endif
  if ((fabs(1 - A / AG) > CTPOBFORCE) ||
      (fabs(1 - E / (EG + 10e-10)) > CTPOBFORCE) ||
      (fabs(1 - SI2 / (IG + 10e-10)) > CTPOBFORCE)) {
    AG = A;
    EG = E;
    IG = SI2;
    JREZ[0] = 0;
    ...
    if (LM > 2)
      QPRT();
#if BOKO
    if ((LT > 0) && (N <= (LT + 0.2) * CBZ))
      REZ();
    if (PLUNASUN)
      LS();
#endif
#if HOKO
    if (PATMOSFERA && (HP < 1500) && (KB0 > 0))
      KATM(HODH, B1, B2)
#endif
          ;
  }
    ...
    */
    {
      ...
      for (K = 0; K < LKL; K++) {
        HL[K] =
            HL0[K] + (((HL4[K] * X + HL3[K]) * X + HL2[K]) * X + HL1[K]) * X;
        if (K < LKS)
          HC[K] =
              HC0[K] + (((HC4[K] * X + HC3[K]) * X + HC2[K]) * X + HC1[K]) * X;
      }
    }
    AAL = sqr(A / ALUN);
    AAC = sqr(A / AC);
  ...
  if ((PATMOSFERA && HP < 1500) || PSUNDAB) {
    ...
    if (PSUNDAB > 0) {
      ...
      L3 += GR * K5_SUN * 0.5 * CI2 / H2 *
            (((1 + E * E) * SK - 0.25 * E * S1) * CW[1] +
             H2 * (CK - 0.25 * E * C1) * SW[1] - 1.5 * E * CW[1] * K1_SUN);
      DOM = GR * K5_SUN / (H2 * 2 * CI2) *
            (((1 + E * E) * SK - 0.25 * E * S1) * SW[1] -
             H2 * (CK - 0.25 * E * C1) * CW[1] - 1.5 * E * K1_SUN * SW[1]);
      L5 += DOM;
      L2 += GR * H2 *
            (0.25 * K3_SUN * H2 * C1 + K4_SUN * (0.25 * S1 - E * SK) +
             1.5 * K4_SUN * K1_SUN);
      JK = 2 * SI2 * DOM;
      K2 = GR * H2 *
           (K3_SUN * (E * SK + 0.25 * S1) + K4_SUN / H2 * (E * CK - 0.25 * C1) -
            1.5 * K3_SUN * K1_SUN);
      KJ = -2 * GR *
           (K3_SUN * ((1 + E * E) * SK - 0.25 * E * S1) -
            K4_SUN * H2 * (CK - 0.25 * E * C1) - 1.5 * E * K3_SUN * K1_SUN);
      L6 += ((1 - H2) / E * K2 + JK + KJ);
      L4 += (E * JK + K2);
    }
#endif
    ...
    if (PATMOSFERA && (HP <= 1500)) {
      ...
      EK[3] = K1 * (0.5 * C3 * (CW[4] * C1 - SW[4] * S1) -
                    0.5 * C2 * (CW[2] * C1 - SW[2] * S1));
      ...
      EK[3] = K1 * (0.5 * C3 * (SW[4] * C1 + CW[4] * S1) -
                    0.5 * C2 * (SW[2] * C1 + CW[2] * S1));
  ...
  // N_moon - ������� ������� ����������� ���� ������ ���� �� ���������.
  // ���������� ��������������� ���������� ����� ���:
  // 15341 -��� ����� ����� �� 31.12.1957 �� 01.01.2000
                      t=(DT0+T-15341)/36525;
                                          l_moon =2.355548393
  +t*(8328.69142288+t*(1.517952e-4 +t*3.103e-7)); l_sun = 6.24003594 +
  t*(628.30195602*t - t*(2.7974e-6+ t*5.82e-8)); N_moon = 2.182438624 + t*(-
  33.757045936 +t*(3.61429e-5+ 3.88e-8*t)); DN_moon=  (- 33.757045936
  +t*(2*3.61429e-5 +3*3.88e-8*t))/36525.0; n_moon = 13.1763965/RADIAN; n_sun =
  0.9856262833676/RADIAN; //?? TE=(21183.5+DT0+T)/36525.0; L_moon=
  4.719966501865+(8399.70915755+ 3.488127776573e-5*TE)*TE;      //???
                                          L_sun=6.2565835+628.30193*TE;
                      k_sec=1/(3600*RADIAN);//����� ������ � 1 �������
                      ...
                      dpsi
  =k_sec*(-17.24*DN_moon*cos(N_moon)+0.42*DN_moon*cos(2*N_moon)-
