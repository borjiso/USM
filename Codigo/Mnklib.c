...
/* ======================================================================
   ...
   Output data: a is square matrix of demension ja x ja, where are placed the
  inversed matrix; P=1 if matrix is singular, else P=0.
  ------------------------------------------------------------------- */
/**
 * @brief matrix inversion.
 * @param  az  square matrix of dimension jaz x jaz.
 * @param  jaz dimension.
 * @param  n demension of left-top submatrix of matrix az to be inversed.
 * @param  a square matrix of dimension ja x ja.
 * @param  ja dimension.
 * @return  int
 */
...
/* ======================================================================
  ...
  ------------------------------------------------------------------- */
/**
 * @brief transition matrix calculation.
 * @param  el el[0:5] is vector of nonsingular elements
 * @param  pme pme[7][7] transition matrix of dimension.
 * @param  delvre is prediction interval in days.
 * @return int
 */
void JVC_DLL vpm(double el[], double pme[7][7], double delvre)
/*  �PO�E��PA PAC�ETA MATP��� �ACTH�X                    */
/*  �PO��BO�H�X (�EPEXO�HO� MATP���)                     */
/*         << B�M (��,�M�,�E�BPE)>>                      */
/*    BXO�: ��[0:6]- BEKTOP �A�PAH�EB�X �O               */
/*          �E�BPE - �HTEPBA� �PO�HO�-�                  */
/*    B�XO�: �M�[6,6]-MATP��A �ACTH�X                    */
/*                    �PO��BO�H�X                        */
{
  int i, j;
  double el0[6], am, ekc, nakl, ash, ash2, pe, n, n1, k0, k1, k2, k3, k4, k5,
      k6, k7, k8, k9, k10, z, J, kk, t, eps, N_moon, sin2I, n_moon_sqr_to_n,
      n_sun_sqr_to_n, sin2eps;
  LBK(el, el0);
  am = el0[0];
  ekc = el0[1];
  nakl = el0[2];
 ...
 * @param  radius no description available.
 * @param  alfa   no description available.
 * @param  delta  no description available.
 * @param  [name] no description available.
 * @return  void      no description available.
 */
void JVC_DLL D_rad_xyz(double radius, double alfa, double delta,
                       double A[2][3]) {
...
/* ======================================================================
  ...
  ------------------------------------------------------------------- */
/**
 * @brief the partial derivative matrix calculation.
 * @param  e e[0:5] is vector of nonsingular elements.
 * @param  D the partial derivative matrix D[6][6].
 * @return  void
 */
//@@@@@@@@@@@@@@@@@@@@@@@@@@NO SE USA@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@//
void JVC_DLL D_xyz_alambda(double e[6], double D[6][6]) {
  double CC[3], SS[3], DCP[3], DCQ[3], DSP[3], DSQ[3], A, L, H, Q, P, l, K, E,
      KCI, ETA, N, NU, R1, T, E0, H1, S, C, C1, S1, C2, S2, CH, CL, SH, SL, DCH,
      DSH, DCL, DSL, CE, SE;
  int I;
  A = e[0];
  L = e[1];
  ...
  NU = 1 / (1 + KCI);
  T = NU * NU / KCI;
  for (E0 = l, E = l + 0.01; fabs(E - E0) > 1.E-8;
       E = l + L * sin(E0) - H * cos(E0))
    E0 = E;
  CE = cos(E);
  SE = sin(E);
  H1 = L * SE - H * CE;
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@NO SE USA@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@//
void JVC_DLL REDUCTION(double alfa, double delta, double JD1957, int Epocha,
                       int Psr, double *d_alfa, double *d_delta) {
  /*
          ��������� ��������� ������� ��������� �� ����� 1950�
          � ����������������� ������� ���������
          ����:
          alfa-������ ����������� �� ����� 1950 � (2000,JD1957)
          delta-��������� �� ����� 1950 � (2000,JD1957)
          Psr-�������
          Epocha =
          0-1950
          1-2000
  ...
  */
  double d, T, L, sin_L, cos_L, sin_alfa, cos_alfa, sin_del, cos_del, kappa,
      d_kappa, d_epsilon, k, d_k;
  ...
  if (Epocha == 1) // Epocha 2000
  {
    ...
    *d_alfa =
        ((kappa + d_kappa) * sin(alfa) - d_epsilon * cos(alfa)) * tan(delta);
    *d_delta = (kappa + d_kappa) * cos(alfa) + d_epsilon * sin(alfa);
    k = (3.505971E-5 + 0.290975E-12 * T + 0.2071E-18 * T * T) * T /
        RADIAN;             /*��������� �� ������� �����������*/
    d_k = -76.7e-6 * sin(L) /*������� � ������ �����������*/
          + 0.9e-6 * sin(2 * L) -
  ...
  if (Psr) {
    d = JD1957 + 21183.5;
    T = d / 36525.0;
    L = (297.696678 + 0.9856473354 * d + 0.000303 * T * T + 1.3966288 * T) /
        360;
    sin_L = sin(L);
    cos_L = cos(L);
    sin_alfa = sin(alfa);
    ...
    sin_del = sin(delta);
    cos_del = cos(delta);
    *d_alfa -= 0.48481368e-5 *
               (20.47 * sin_alfa * sin_L + 18.87 * cos_L * cos_alfa) / cos_del;
    *d_delta -= 0.48481368e-5 *
                (20.47 * sin_del * sin_L * cos_alfa +
                 18.87 * cos_L * (0.4336661 * cos_del - sin_del * sin_alfa));
  }
}
/* ================================================================ */
...
void JVC_DLL KVAZI_TO_T(double X[], double T, int p) {
  ...
  */
  /* ======================================================================
     ...
     quasinertial coordinate system to the coordinate system  related to the
    epoch of date Call: KVAZI_TO_T(X,T,P); Input data: X[0:5] is the position
    and velocity vector in the quasinertial system coordinate; T is the time
    counted off from 0h UTC Dec 31 1957. if P=0 then the output data should be
    produced in the True Eqinox and True Equator of Date frame, else the output
    data should be produced in the Mean Eqinox and Mean Equator of Date frame.
  ...
  k = (3.505971E-5 + 0.290975E-12 * T + 0.2071E-18 * T * T) * T /
      RADIAN; /*��������� �� ������� �����������*/
  dk_dt = (3.505971E-5 + 2 * 0.290975E-12 * T + 3 * 0.2071E-18 * T * T) /
          (RADIAN * 86400.0); /*�������� ��������� �� ������� �����������*/
  L = (12.1128 - 0.0529539 * T) / RADIAN;
  d_k = -76.7e-6 * sin(L) /*������� � ������ �����������*/
        + 0.9e-6 * sin(2 * L) -
...
/* ======================================================================
   Destination: transformation of the position and velocity in the
   quasinertial coordinate system to the coordinate system  related to the epoch
  of date Call: T_TO_KVAZI(X,T,P) Input data: X[0:5] is the position and
  velocity vector in the coordinate system  related to the Epoch of Date. T is
  the time counted off from 0h UTC Dec 31 1957. if P=0 then the input data are
  presented in the True Eqinox and Equator of Date, else the input data are
  presented in the Mean Equator of Date. Output data:X[0:5] is the position and
  velocity vector in the quasinertial system coordinate.
  ------------------------------------------------------------------- */
//@@@@@@@@@@@@@@@@@@@@@@@@@@NO SE USA@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@//
...
*/
{
  ...
  *LL = 0.5 * atan(k * sin(2 * eekl) /
                   (-2 * C20 * sqr(RZ / A * n) + k * cos(2 * eekl)));
  c1 = sin(I) * sin(DBY);
...
/* ================================================================ */
//@@@@@@@@@@@@@@@@@@@@@@@@@@NO SE USA@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@//
void JVC_DLL PROGNOZ_KMO(double EL[], double KMO[7][7], double DELT,
                         double EK[], double radius, double alfa, double delta,
                         int P, double C[7][7], double C2[2][2])
/*
   ��������� ��������������� ��� ��
   ����:
   EL[6]-������ ����������� ��������� �� ������ t0
...
/* ����������: ��� �=1 : �������� gmsds �� ����������� ������� (�),������ (�),
                         ������� (�),���� ������ (�)[���.��.��.��] � rdr [���]
                   k=0         [���] - > [���.��.��.��]
   ������    : gmsds = 135.54.39.98 - ��� 135 ����., 54 ��� ,39.98 ��� .
   !!!! ���� �� -90 �� 90 ��������
*/
{
  int I, g, m;
