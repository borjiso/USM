...
extern int LM, LT, LKL, LKS, JREZ[MAXREZ], L, M, PLUNASUN
...
extern double
    ...
    A,
    E, I, SI, CI, SI2, CI2, CE, SE, EA, IA, CIA, SIA, AAL, AAC, TK, S0, THAX,
    TKOH;
...
extern double AX, EX, IX
    ...
    ;
double G[LMAX + 1][2 * LMAX + 1][2], far *FF
#if BOKO
    ,
...
double Y[6] /*     [6]      */
    ...
    EK[6] /*     [6]      */
    ;
double W, U, V, X, K13, MK, AK, AP, DBY, N13, CA, H2, N, Z, CM, SK, CK, FF1, H1,
    H3, N1, N2, N3, N4, N5, N6, N7, N8, N9, N10, FF2, CLM, SLM, K0, K1, K2, K3,
    K4, K5, K6, K7, K8, K9, F1, F2, N11, N12, SN, TI2, DOW, DW, DM, G1, G2,
    RADIUS, dS;
int CQ, P, PF, QMM, J1, K, LK, J, L0, L1, Q, S, N_korper;

/*########################## ������ FINC2 ###########################*/
/* ======================================================================
   Destination: Calculation of all inclination and its derivatives up to order
  L2. Call: FINC2(L2). It is used the global variables SI2=sin(.5*i);
  CI2=cos(.5*i); Output data: dynamic array FF with value of inclination
 ...
 * @param L2 order of derivative.
 */
void FINC2(int L2) {
  double A1, A11, A2, A22, A3, A33, R1, R2, R3, R4, R5, R6;
  int J, N, K, I, M, P;
...
int JVC_DLL SKP(int PKP, double far EBX[], double T, double far E1[]) {
  ...
  N6 = CIA * N2 + N1 * SIA;
  K1 = C20 *
       (sqr((1 + E * CIA) * H3) * (1 + E * CIA) * H3 * (3 * Z * N6 * N6 - 1) -
        0.5 * (3 * Z - 2) * H1 * H3);
  K2 = C20 *
       (3 * (N5 * N5 - N6 * N6) +
        E * ((4 * N5 * N5 - 3) * N5 * N1 + (3 - 4 * N6 * N6) * N6 * N2) +
        3 * E * (N5 * N1 - N2 * N6)) *
       H3 * 0.25;
  N7 = N1 * N1 - N2 * N2;
  N8 = 2 * N1 * N2;
  ...
  FF1 = Z * Z;
  E1[0] =
      A * K1 * CM * (1 - K1 * CM - 0.75 * G2 * (3 * Z - 2) * H3 * H1) -
      1.5 * CM * G2 * A * (4 - 5 * Z) * Z * K2 * G1 * H1 -
      3.0 / 64.0 * G2 * G2 * A * G1 * H3 * H1 *
          ((-8 + 8 * Z + 5 * FF1) * H2 * H2 + 4 * (4 - 12 * Z + 9 * FF1) * H2 -
           5 * (-8 + 16 * Z - 7 * FF1) + 2 * (-14 * Z + 15 * FF1) * E * E * N7);
  ...
  N10 = N8 * CIA - SIA * N7;
  K1 = 1.5 * G2 * CI * G1 *
       (IA - CA + E * SIA - 0.5 * (N8 * N1 + N7 * N2) - E / 6 * N11 -
        E / 2 * N9);
  N12 = N8 * (N1 * N5 - N2 * N6) + (N2 * N5 + N1 * N6) * N7;
  N13 = N8 * (N1 * N1 - N2 * N2) + 2 * N2 * N1 * N7;
  K3 = 0.75 * G2 * G1 *
       ((5 * Z - 4) * (IA - CA) +
        (3 * FF1 + 17.0 / 4.0 * FF2 - 2 / E - 3.5 * E) * SIA +
        0.125 * FF2 * N10 + (0.5 * FF1 - 15.0 / 8.0 * FF2 + E) * N9 +
        (1.5 * Z - 1) * N2 + (1 - 2.5 * Z) * (N8 * N1 + N7 * N2) +
        (0.25 * FF2 - E / 6) * N6 +
        (E / 3 - 7 * FF1 / 6 - 19.0 / 24.0 * FF2) * N11 -
  ...
  F1 =
      0.75 * G2 * H3 * H1 *
      ((0.75 * FF2 - 3 * FF1 + 2 / E - 0.5 * E) * SIA + (1 - 1.5 * Z) * N2 +
       (E / 6 - FF2 / 4) * N6 - 0.125 * FF2 * N10 -
       (0.5 * FF1 + 5.0 * FF2 / 8.0) * N9 + (7.0 * FF1 / 6.0 - FF2 / 24) * N11 +
       0.75 * Z * (N13 - N8) + 0.125 * FF2 * N12);
  E1[5] = K13 + F1;
  CM = G2 * G1;
  DOW = 1.5 * CM * CI;
  ...
  if (PKP == 0) {
    ...
    F1 = 0.75 * G2 * H3 * H1 *
         ((0.75 * FF2 - 3 * FF1 + 2 / E - 0.5 * E) * SIA + (1 - 1.5 * Z) * N2 +
          (E / 6 - FF2 / 4) * N6 - 0.125 * FF2 * N10 -
          (0.5 * FF1 + 5.0 * FF2 / 8.0) * N9 +
          (7.0 * FF1 / 6.0 - FF2 / 24) * N11 + 0.75 * Z * (N13 - N8) +
          0.125 * FF2 * N12);
    E1[5] = K13 + F1;
    CM = G2 * G1;
    DOW = 1.5 * CM * CI;
  ...
  if (PKP != 0) {
    ...
    N5 = N1 * CIA - N2 * SIA;
    N6 = CIA * N2 + N1 * SIA;
    K1 = C20 * (sqr((1 + K0 * CIA) * K5) * ((1 + K0 * CIA) * K5) *
                    (3 * Z * N6 * N6 - 1) -
                0.5 * (3 * Z - 2) * K3 * K5);
    K2 = C20 *
         (3 * (N5 * N5 - N6 * N6) +
          K0 * ((4 * N5 * N5 - 3) * N5 * N1 + (3 - 4 * N6 * N6) * N6 * N2) +
          3 * K0 * (N5 * N1 - N2 * N6)) *
         K5 * 0.25;
    G2 = CM * C20;
    N8 = 2 * N1 * N2;
    G2 = CM * C20;
    FF1 = Z * Z;
    E1[0] = Y[0] * K1 * CM * (1 - K1 * CM - 0.75 * G2 * (3 * Z - 2) * K5 * K3) -
            1.5 * CM * G2 * Y[0] * (4 - 5 * Z) * Z * K2 * G1 * K3 -
            3.0 / 64.0 * G2 * G2 * Y[0] * G1 * K5 * K3 *
                ((-8 + 8 * Z + 5 * FF1) * K4 * K4 +
                 4 * (4 - 12 * Z + 9 * FF1) * K4 - 5 * (-8 + 16 * Z - 7 * FF1) +
                 2 * (-14 * Z + 15 * FF1) * K0 * K0 * N7);
  }
  /*����� ������� �������� ��������� -24.02.93*/
  ...
  if ((LM > 2) || (LT != 0) || PLUNASUN) {
...

    {
      ...
      if ((fabs(1 - A / AX) > CTPOBKP) ||
          (fabs(1 - E / (EX + 10e-10)) > CTPOBKP) ||
          (fabs(1 - SI2 / (IX + 10e-10)) > CTPOBKP)) {
        ;
        if (!PLUNASUN)
          FINC2(L);
        rewind(MULTIINDEX);
            ...
            for (S = -LM; S <= LM; S++) {
              ...
              for (M = 0; M <= LT; M++) {
                ...
                if (fabs(S + Q - M * CBZ / N) > 5.0E-2) {
                  ...
                  DKP[6] *= N1 * N8;
                  DKP[7] *= N1 * N8;
                  if (((fabs(DKP[0]) < EPSKP) && (fabs(DKP[1]) < EPSKP) &&
                       (fabs(DKP[2]) < EPSKP) && (fabs(DKP[3]) < EPSKP) &&
                       (fabs(DKP[4]) < EPSKP) && (fabs(DKP[5]) < EPSKP) &&
                       (fabs(DKP[6]) < EPSKP) && (fabs(DKP[7]) < EPSKP) &&
                       (fabs(DKP[8]) < EPSKP) && (fabs(DKP[9]) < EPSKP) &&
                       (fabs(DKP[10]) < EPSKP) && (fabs(DKP[11]) < EPSKP))) {
      ...
      for (L = 0; L < N_korper; L++) {
        if (fread(&S, sizeof(S), 1, MULTIINDEX) &&
          ...
          CK = cos(K0);
          for (J = 0; J <= 5; J++)
            EK[J] += DKP[2 * J] * CK + DKP[2 * J + 1] * SK

                /*
                #if PECHAT
                ;printf("\nS=%d Q=%d M=%d SK=%11.8f CK=%11.8f \n
                DKP(0-05)=",S,Q,M,SK,CK); ;  for(J=0;J<=5;J++)  printf("%10.6e
                ",DKP[J]); printf("\n DKP(6-11)="); ;  for(J=0;J<=5;J++)
                printf("%10.6e ",DKP[J+6]); N_korper++ #endif
                */
                ;
        } else
          return (4);
    ...
    if (PLUNASUN) {
       ...
       */
      {
        ...
        for (K = 0; K < LKL; K++) {
          ...
          if (K < LKS) {
            HC[K] = HC0[K] +
                    (((HC4[K] * X + HC3[K]) * X + HC2[K]) * X + HC1[K]) * X;
            DC[K] =
                HC1[K] + ((4 * HC4[K] * X + 3 * HC3[K]) * X + 2 * HC2[K]) * X;
          ...
          for (P = 0; P <= L; P++) {
            for (Q = -QMM; Q <= QMM; Q++) {
              ...
              if (S != 0) {
                ...
                N3 = FF2 * G1 * SLM;
                ;
                if ((fabs(K1 / E) >= 1E-12) || (fabs(K1 / SI) >= 1E-12) ||
                    (fabs(K2) >= 1E-8) || (fabs(K3) >= 1E-8) ||
                    (fabs(N1 / E) >= 1E-12) || (fabs(N1 / SI) >= 1E-12) ||
                    (fabs(N2) >= 1E-8) || (fabs(N3) >= 1E-8)) {
                  ...
                  K4 =
                      U * (J * CK + (1 - J) * SK) + V * (J * SK + (J - 1) * CK);
                  K5 = U * (-J * SK + (1 - J) * CK) +
                       V * (J * CK + (1 - J) * SK);
                  N4 =
                      W * (J * CK + (1 - J) * SK) + Z * (J * SK + (J - 1) * CK);
                  N5 = W * (-J * SK + (1 - J) * CK) +
                       Z * (J * CK + (1 - J) * SK);
                  K0 = K1 * K5 + N1 * N5;
                  K1 = K1 * K4 + N1 * N4;
